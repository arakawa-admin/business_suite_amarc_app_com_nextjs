...
export async function seedMasterInsuranceCategories() {
...

    {
        ...
        if (error) {
            console.error(
                "Delete master_insurance_categories error:",
                error,
            );
            throw error;
        }
...
②指示に従い対応`,
    ...
    for (const category of categories) {
        ...
        if (error) {
            console.error(
                "Insert master_insurance_categories error:",
                error,
            );
            throw error;
        }
    ...
    console.log("master_insurance_categories seeded successfully!");
}

export async function seedMasterInsuranceAgencies() {
    const supabase = createClient(
        process.env.SUPABASE_URL,
        process.env.SUPABASE_SERVICE_ROLE_KEY,
    );
...

    {
        ...
        if (error) {
            console.error(
                "Delete master_insurance_agencies error:",
                error,
            );
            throw error;
        }
    ...
    if (categoryError) {
        console.error(
            "Select master_insurance_categories error:",
            categoryError,
        );
        throw categoryError;
    }
    ...
    for (const agency of agencies) {
        ...
        if (!insuranceCategoryId) {
            throw new Error(
                `insurance_category_code=${agency.insurance_category_code} に対応するカテゴリが存在しません。先に seedMasterInsuranceCategories() を実行してください。`,
            );
        }
        ...
        if (error) {
            console.error(
                "Insert master_insurance_agencies error:",
                error,
            );
            throw error;
        }
    ...
    console.log("master_insurance_agencies seeded successfully!");
}
