...
import {
    fetchApprovalOrders,
    fetchApprovalOrdersByApprovalId,
    fetchApprovalOrderById,
    insertApprovalOrder,
    updateApprovalOrderById,
} from "@/lib/repositories/approval/approvalOrder.repository";

export async function getApprovalOrders() {
    try {
        const res = await fetchApprovalOrders();
        if (!res.ok) {
            return { ok: false as const, error: "稟議承認者一覧の取得に失敗しました" };
        }
...
export async function getApprovalOrdersByApprovalId(id: string) {
    try {
        const res = await fetchApprovalOrdersByApprovalId(id);
        if (!res.ok) {
            return { ok: false as const, error: "稟議承認者一覧の取得に失敗しました" };
        }
        return { ok: true as const, data: res.data || [] };
...
export async function getApprovalOrderById(id: string) {
    try {
        ...
        if (!res.ok) return { ok: false as const, error: "稟議承認者の詳細取得に失敗しました" };
        if (!res.data) return { ok: false as const, error: "稟議承認者が見つかりません" };
        return { ok: true as const, data: res.data };
    } catch (e: any) {
        return { ok: false as const, error: e.message };
    }
...
export async function createApprovalOrder(data: ApprovalOrderCreateInput) {
    try {
        ...
        if (!res.ok) { return { ok: false as const, error: "稟議承認者の登録に失敗しました" }; }
...
export async function updateApprovalOrder(
    ...
    try {
        ...
        if (!res.ok) { return { ok: false as const, error: "稟議承認者の更新に失敗しました" }; }
