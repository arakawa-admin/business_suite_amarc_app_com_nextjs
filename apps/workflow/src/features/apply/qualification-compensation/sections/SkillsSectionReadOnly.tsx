...
import { Grid, Stack, Typography, Paper, Chip } from "@mui/material";

import { format } from "date-fns";
import { QualificationCompensationInput } from "../zod";
...
export default function SkillsSectionReadOnly({
    ...
    skills: QualificationCompensationInput["skills"]
                                    ...
                                    <Stack spacing={1}>
                                        <Typography variant="body2">
                                            <Stack direction="row" spacing={1} alignItems="center" sx={{ mb: 1 }}>
                                                <Chip label={skill.is_add ? "追加" : "削除"} size="small" component={"span"} color={skill.is_add ? "success" : "error"} sx={{ mr: 1 }} />
                                                <div>適用月: {format(skill.applicable_month, "yyyy年MM月")}</div>
                                                <div>技能名: {skill.skill_name}</div>
                                            </Stack>
                                        </Typography>
                                        <UploadedFileList
