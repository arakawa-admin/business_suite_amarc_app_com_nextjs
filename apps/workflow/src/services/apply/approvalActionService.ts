...
import { updateApplication } from "@/lib/actions/apply/application";
import { createApprovalAction, updateApprovalAction } from "@/lib/actions/apply/approvalAction";
import { getApprovalOrdersByApplicationId, updateApprovalOrder } from "@/lib/actions/apply/approvalOrder";
...
async function approvalProcess(status: MasterStatusType[], data: any) {
    ...
    const orders = await getApprovalOrdersByApplicationId(data.application_id);
    if (!orders.ok) {
        console.error("getApprovalOrdersByApplicationId failed", orders.error);
    }
    if (orders.data) {
        // *** 次の人に承認ターン移動 *** //
        ...
        if (nextOrder) {
            ...
            // TODO パート社員雇用継続申請、契約社員雇用継続申請、試用期間満了申請、再雇用希望申請 の場合
            // TODO 対象ユーザの雇用期間満了日を更新
        }
    }
}
