...
import { getMasterDepartmentReviewers, enableMasterDepartmentReviewer, disableMasterDepartmentReviewer } from '@/lib/actions/approval/masterDepartmentReviewer';
...
export default function ClientPageMasterStaff({
    ...
    initial: MasterDepartmentReviewerType[]
}) {
    const TITLE = "回議者マスタ"
    ...
    const {
        data: result,
        mutate,
    } = useSWR<FetchResult<MasterDepartmentReviewerType[]>>(
        'approval/master-department-reviewers',
        getMasterDepartmentReviewers,
        {
    ...
    const [editItem, setEditItem] = useState({} as MasterDepartmentReviewerType);
    const handleCreate = () => {
        setCreateOpen(true);
        setIsCreate(true);
    }
    const handleEdit = (editItem: MasterDepartmentReviewerType) => {
    ...
    const cancelCreate = () => {
        setCreateOpen(false);
        setEditItem({} as MasterDepartmentReviewerType);
    }

    // 無効化
    const [deleteItem, setDeleteItem] = useState({} as MasterDepartmentReviewerType);
    const [confirmDeleteOpen, setConfirmDeleteOpen] = useState(false);
    const handleDelete = (staff: MasterDepartmentReviewerType) => {
        setDeleteItem(staff);
        setConfirmDeleteOpen(true);
    }
    const doDelete = async () => {
        try{
            await disableMasterDepartmentReviewer(deleteItem.id);
            mutate();
    ...
    const cancelDelete = () => {
        setDeleteItem({} as MasterDepartmentReviewerType);
        setConfirmDeleteOpen(false);
    }

    // 有効化
    const [restoreItem, setRestoreItem] = useState({} as MasterDepartmentReviewerType);
    const [confirmRestoreOpen, setConfirmRestoreOpen] = useState(false);
    const handleRestore = (staff: MasterDepartmentReviewerType) => {
        setRestoreItem(staff);
        setConfirmRestoreOpen(true);
    }
    const doRestore = async () => {
        try{
            await enableMasterDepartmentReviewer(restoreItem.id);
            mutate();
    ...
    const cancelRestore = () => {
        setRestoreItem({} as MasterDepartmentReviewerType);
