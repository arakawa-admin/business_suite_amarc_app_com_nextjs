...
import {
    masterDepartmentApproverUpsertSchema,
    masterDepartmentApproverUpdatePayloadSchema,
    type MasterDepartmentApproverUpsertInput,
    type MasterDepartmentApproverUpdateInput,
    isUpdateInput,
...
import { createMasterDepartmentApprover, updateMasterDepartmentApprover } from "@/lib/actions/approval/masterDepartmentApprover";

export default function FormMasterDepartmentApprover({
    masters,
    editValues,
    onSubmitted,
    onClose,
}: {
    masters: MasterDepartmentApproverType[]|undefined
    editValues?: MasterDepartmentApproverType
    onSubmitted: () => void
    onClose: () => void
}) {
    ...
    const baseDefaults = useMemo<MasterDepartmentApproverUpsertInput>(() => ({
        department_id: "",
        sequence: 1,
        approver_user_id: "",
        remarks: "",
    ...
    const normalizedEdit: MasterDepartmentApproverUpsertInput | null = useMemo(() => {
        ...
        return {
            ...
            valid_at: toDateOrUndefined((editValues as MasterDepartmentApproverUpdateInput).valid_at),
            invalid_at: toDateOrUndefined((editValues as MasterDepartmentApproverUpdateInput).invalid_at),
        };
    }, [isUpdate, editValues, baseDefaults]);

    const methods = useForm<MasterDepartmentApproverUpsertInput>({
        resolver: zodResolver(masterDepartmentApproverUpsertSchema),
    ...
    const validateSequenceByDepartment = (values: MasterDepartmentApproverUpsertInput) => {
    ...
    const [formData, setFormData] = useState<MasterDepartmentApproverUpsertInput | null>(null);
    ...
    const onSubmit = async (data: MasterDepartmentApproverUpsertInput) => {
        start(async () => {
            try {
                if (isUpdateInput(data)) {
                    ...
                    const { id, ...rest } = data;
                    // 型/値検証（任意だが安全）
                    const payload = masterDepartmentApproverUpdatePayloadSchema.parse(rest);
                    const res = await updateMasterDepartmentApprover(id, payload);
                    if (!res.ok) {
                    ...
                    const res = await createMasterDepartmentApprover(data);
                    if (!res.ok) {
                        toast.error(res.error);
                        onClose();
                        return;
