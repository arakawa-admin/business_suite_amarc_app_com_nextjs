...
import { getMasterDepartmentApprovers, enableMasterDepartmentApprover, disableMasterDepartmentApprover } from '@/lib/actions/approval/masterDepartmentApprover';
...
export default function ClientPageMasterStaff({
    ...
    initial: MasterDepartmentApproverType[]
}) {
    const TITLE = "承認者マスタ"
    ...
    const {
        data: result,
        mutate,
    } = useSWR<FetchResult<MasterDepartmentApproverType[]>>(
        'approval/master-department-approvals',
        getMasterDepartmentApprovers,
        {
    ...
    const [editItem, setEditItem] = useState({} as MasterDepartmentApproverType);
    const handleCreate = () => {
        setCreateOpen(true);
        setIsCreate(true);
    }
    const handleEdit = (editItem: MasterDepartmentApproverType) => {
    ...
    const cancelCreate = () => {
        setCreateOpen(false);
        setEditItem({} as MasterDepartmentApproverType);
    }

    // 無効化
    const [deleteItem, setDeleteItem] = useState({} as MasterDepartmentApproverType);
    const [confirmDeleteOpen, setConfirmDeleteOpen] = useState(false);
    const handleDelete = (staff: MasterDepartmentApproverType) => {
        setDeleteItem(staff);
        setConfirmDeleteOpen(true);
    }
    const doDelete = async () => {
        try{
            await disableMasterDepartmentApprover(deleteItem.id);
            mutate();
    ...
    const cancelDelete = () => {
        setDeleteItem({} as MasterDepartmentApproverType);
        setConfirmDeleteOpen(false);
    }

    // 有効化
    const [restoreItem, setRestoreItem] = useState({} as MasterDepartmentApproverType);
    const [confirmRestoreOpen, setConfirmRestoreOpen] = useState(false);
    const handleRestore = (staff: MasterDepartmentApproverType) => {
        setRestoreItem(staff);
        setConfirmRestoreOpen(true);
    }
    const doRestore = async () => {
        try{
            await enableMasterDepartmentApprover(restoreItem.id);
            mutate();
    ...
    const cancelRestore = () => {
        setRestoreItem({} as MasterDepartmentApproverType);
