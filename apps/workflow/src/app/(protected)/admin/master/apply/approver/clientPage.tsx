...
import { getMasterFormApprovers, enableMasterFormApprover, disableMasterFormApprover } from '@/lib/actions/apply/masterFormApprover';
...
export default function ClientPageMasterFormApprover({
    initial,
}: {
    initial: MasterFormApproverType[]
}) {
    const TITLE = "承認者マスタ"
    ...
    const {
        data: result,
        mutate,
    } = useSWR<FetchResult<MasterFormApproverType[]>>(
        'apply/master-form-applys',
        getMasterFormApprovers,
        {
    ...
    const [editItem, setEditItem] = useState({} as MasterFormApproverType);
    const handleCreate = () => {
        setCreateOpen(true);
        setIsCreate(true);
    }
    const handleEdit = (editItem: MasterFormApproverType) => {
    ...
    const cancelCreate = () => {
        setCreateOpen(false);
        setEditItem({} as MasterFormApproverType);
    }

    // 無効化
    const [deleteItem, setDeleteItem] = useState({} as MasterFormApproverType);
    const [confirmDeleteOpen, setConfirmDeleteOpen] = useState(false);
    const handleDelete = (staff: MasterFormApproverType) => {
        setDeleteItem(staff);
        setConfirmDeleteOpen(true);
    }
    const doDelete = async () => {
        try{
            await disableMasterFormApprover(deleteItem.id);
            mutate();
    ...
    const cancelDelete = () => {
        setDeleteItem({} as MasterFormApproverType);
        setConfirmDeleteOpen(false);
    }

    // 有効化
    const [restoreItem, setRestoreItem] = useState({} as MasterFormApproverType);
    const [confirmRestoreOpen, setConfirmRestoreOpen] = useState(false);
    const handleRestore = (staff: MasterFormApproverType) => {
        setRestoreItem(staff);
        setConfirmRestoreOpen(true);
    }
    const doRestore = async () => {
        try{
            await enableMasterFormApprover(restoreItem.id);
            mutate();
    ...
    const cancelRestore = () => {
        setRestoreItem({} as MasterFormApproverType);
