...
import { getApplyFormCategorys, enableApplyFormCategory, disableApplyFormCategory } from '@/lib/actions/apply/applyFormCategory';
...
export default function ClientPageApplyFormCategory({
    initial,
}: {
    initial: ApplyFormCategoryType[]
}) {
    const TITLE = "申請書マスタ"
    ...
    const [editItem, setEditItem] = useState({} as ApplyFormCategoryType);
    const handleCreate = () => {
        setCreateOpen(true);
        setIsCreate(true);
    }
    const handleEdit = (editItem: ApplyFormCategoryType) => {
    ...
    const cancelCreate = () => {
        setCreateOpen(false);
        setEditItem({} as ApplyFormCategoryType);
    }

    // 無効化
    const [deleteItem, setDeleteItem] = useState({} as ApplyFormCategoryType);
    const [confirmDeleteOpen, setConfirmDeleteOpen] = useState(false);
    const handleDelete = (staff: ApplyFormCategoryType) => {
        setDeleteItem(staff);
        setConfirmDeleteOpen(true);
    }
    const doDelete = async () => {
        try{
            await disableApplyFormCategory(deleteItem.id);
            mutate();
    ...
    const cancelDelete = () => {
        setDeleteItem({} as ApplyFormCategoryType);
        setConfirmDeleteOpen(false);
    }

    // 有効化
    const [restoreItem, setRestoreItem] = useState({} as ApplyFormCategoryType);
    const [confirmRestoreOpen, setConfirmRestoreOpen] = useState(false);
    const handleRestore = (staff: ApplyFormCategoryType) => {
        setRestoreItem(staff);
        setConfirmRestoreOpen(true);
    }
    const doRestore = async () => {
        try{
            await enableApplyFormCategory(restoreItem.id);
            mutate();
    ...
    const cancelRestore = () => {
        setRestoreItem({} as ApplyFormCategoryType);
