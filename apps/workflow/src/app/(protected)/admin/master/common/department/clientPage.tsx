...
import { getMasterDepartments, enableMasterDepartment, disableMasterDepartment } from '@/lib/actions/common/masterDepartment';
...
import FormMasterDepartment from './Form';
...
export default function ClientPageMasterDepartment({
    initial,
}: {
    initial: MasterDepartmentType[]
}) {
    const TITLE = "部門マスタ"
    ...
    const [editItem, setEditItem] = useState({} as MasterDepartmentType);
    const handleCreate = () => {
        setCreateOpen(true);
        setIsCreate(true);
    }
    const handleEdit = (editItem: MasterDepartmentType) => {
    ...
    const cancelCreate = () => {
        setCreateOpen(false);
        setEditItem({} as MasterDepartmentType);
    }

    // 無効化
    const [deleteItem, setDeleteItem] = useState({} as MasterDepartmentType);
    const [confirmDeleteOpen, setConfirmDeleteOpen] = useState(false);
    const handleDelete = (department: MasterDepartmentType) => {
        setDeleteItem(department);
        setConfirmDeleteOpen(true);
    }
    const doDelete = async () => {
        try{
            await disableMasterDepartment(deleteItem.id);
    ...
    const cancelDelete = () => {
        setDeleteItem({} as MasterDepartmentType);
        setConfirmDeleteOpen(false);
    }

    // 有効化
    const [restoreItem, setRestoreItem] = useState({} as MasterDepartmentType);
    const [confirmRestoreOpen, setConfirmRestoreOpen] = useState(false);
    const handleRestore = (department: MasterDepartmentType) => {
        setRestoreItem(department);
        setConfirmRestoreOpen(true);
    }
    const doRestore = async () => {
        try{
            await enableMasterDepartment(restoreItem.id);
            mutate();
    ...
    const cancelRestore = () => {
        setRestoreItem({} as MasterDepartmentType);
