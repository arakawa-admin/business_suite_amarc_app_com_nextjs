...
import { partTimeEmploymentDefinition } from "@/features/apply/part-time-employment/definition";
import { partTimeEmploymentSchema, type PartTimeEmploymentInput } from "@/features/apply/part-time-employment/zod";
...
import SkillsSectionReadOnly from "@/features/apply/part-time-employment/sections/SkillsSectionReadOnly";
...
export default async function PartTimeEmploymentDetailPage({ params }: Props) {
    ...
    const revisions = revisionsRaw
        .map((r: ApplicationRevisionType) => {
            ...
            const parsed = partTimeEmploymentSchema.safeParse(payload);
            return {
                id: String(r.id),
                revision_no: Number(r.revision_no),
                submitted_at: r.submitted_at ?? null,
                payload: (parsed.success ? parsed.data : payload) as PartTimeEmploymentInput,
        ...
        return (
            ...
            <div style={{display: "flex", justifyContent: "space-between" }}>
                <ApplyBreadcrumbs
                    items={[
                        { title: APPLY_NAME, href: '/apply/part-time-employment' },
                        { title: "申請書 詳細" },
                    ]}
                    />
            ...
            <ReadOnlyRendererRevisions<PartTimeEmploymentInput>
                fields={partTimeEmploymentDefinition.fields}
                revisions={revisions}
