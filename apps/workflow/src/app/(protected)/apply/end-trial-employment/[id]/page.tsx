...
import { endTrialEmploymentDefinition } from "@/features/apply/end-trial-employment/definition";
...
import SkillsSectionReadOnly from "@/features/apply/end-trial-employment/sections/SkillsSectionReadOnly";
...
export default async function EndTrialEmploymentDetailPage({ params }: Props) {
    ...
    const revisions = revisionsRaw
        .map((r: ApplicationRevisionType) => {
            ...
            const parsed = endTrialEmploymentSchema.safeParse(payload);
            return {
                id: String(r.id),
                revision_no: Number(r.revision_no),
                submitted_at: r.submitted_at ?? null,
                payload: (parsed.success ? parsed.data : payload) as EndTrialEmploymentInput,
        ...
        return (
            ...
            <div style={{display: "flex", justifyContent: "space-between" }}>
                <ApplyBreadcrumbs
                    items={[
                        { title: APPLY_NAME, href: '/apply/end-trial-employment' },
                        { title: "申請書 詳細" },
                    ]}
                    />
            ...
            <ReadOnlyRendererRevisions<EndTrialEmploymentInput>
                fields={endTrialEmploymentDefinition.fields}
                revisions={revisions}
