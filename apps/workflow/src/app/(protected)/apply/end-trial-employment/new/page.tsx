...
import EndTrialEmploymentForm from "@/features/apply/end-trial-employment/Form";
import type { EndTrialEmploymentInput } from "@/features/apply/end-trial-employment/zod";
import { submitEndTrialEmployment, updateCurrentRevisionSkillsPayload } from "./actions";
...
export default function Page() {
    ...
    const FORM_CODE = "end-trial-employment";

    const handleSubmit = async (values: EndTrialEmploymentInput) => {
        // Next.js（App Router の Server Actions）はデフォルトで リクエストボディが 1MB 制限につき
        // ファイルそのものはserver componentに渡さないように。
        const { skills, ...rest } = values;

        const res = await submitEndTrialEmployment({
            input: rest,
    ...
    return (
        <ApplicationFormWrapper
            ...
            formCode={FORM_CODE}
            >
            <EndTrialEmploymentForm onSubmit={handleSubmit} />
        </ApplicationFormWrapper>
    );
}
