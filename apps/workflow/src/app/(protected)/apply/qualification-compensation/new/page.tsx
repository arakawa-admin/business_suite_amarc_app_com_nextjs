...
import QualificationCompensationForm from "@/features/apply/qualification-compensation/Form";
import type { QualificationCompensationInput } from "@/features/apply/qualification-compensation/zod";
import { submitQualificationCompensation, updateCurrentRevisionSkillsPayload } from "./actions";
...
export default function Page() {
    ...
    const APPLY_NAME = "資格手当申請";
    const FORM_CODE = "qualification-compensation";

    const handleSubmit = async (values: QualificationCompensationInput) => {
        // Next.js（App Router の Server Actions）はデフォルトで リクエストボディが 1MB 制限につき
        // ファイルそのものはserver componentに渡さないように。
        const { skills, ...rest } = values;

        const res = await submitQualificationCompensation({
            input: rest,
        ...
        if (skills && skills?.length > 0) {
            ...
            const nextSkillsPayload = uploadResults.map(({ skill, fileInputs }) => {
                ...
                return {
                    ...
                    applicable_month: skill.applicable_month.toISOString(), // Dateは文字列に変換して渡す
                };
            });
    ...
    return (
        <ApplicationFormWrapper
            ...
            formCode={FORM_CODE}
            >
            <QualificationCompensationForm onSubmit={handleSubmit} />
        </ApplicationFormWrapper>
    );
}
