...
import ContractEmploymentForm from "@/features/apply/contract-employment/Form";
import type { ContractEmploymentInput } from "@/features/apply/contract-employment/zod";
import { submitContractEmployment, updateCurrentRevisionSkillsPayload } from "./actions";
...
export default function Page() {
    ...
    const FORM_CODE = "contract-employment";

    const handleSubmit = async (values: ContractEmploymentInput) => {
        // Next.js（App Router の Server Actions）はデフォルトで リクエストボディが 1MB 制限につき
        // ファイルそのものはserver componentに渡さないように。
        const { skills, ...rest } = values;

        const res = await submitContractEmployment({
            input: rest,
    ...
    return (
        <ApplicationFormWrapper
            ...
            formCode={FORM_CODE}
            >
            <ContractEmploymentForm onSubmit={handleSubmit} />
        </ApplicationFormWrapper>
    );
}
