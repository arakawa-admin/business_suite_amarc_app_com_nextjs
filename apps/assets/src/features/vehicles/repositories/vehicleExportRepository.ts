...
export type VehicleExportFilterParams = {
    keyword?: string | null;
...
export async function findVehiclesForExport(
    params: VehicleExportFilterParams,
): Promise<VehicleExportRow[]> {
    ...
    if (params.keyword?.trim()) {
        ...
        query = query.or(
            [
                ...
                `registration_number.ilike.%${keyword}%`,
                `note.ilike.%${keyword}%`,
            ].join(","),
        );
    }
    ...
    if (params.includeDeleted) {
        // v_vehicles_list が deleted_at is null 固定なら、ここは将来拡張用。
        // 今は何もしない。
    }
    ...
    if (error) {
        throw new Error(
            `車両CSV出力データの取得に失敗しました: ${error.message}`,
        );
    }

    return (data ?? []) as VehicleExportRow[];
}
