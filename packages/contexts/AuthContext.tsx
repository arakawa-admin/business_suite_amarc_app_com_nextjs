...
type StaffDepartmentType = {
    ...
    mailing_list: string;
    remarks?: string | null;
    valid_at?: Date | undefined;
    invalid_at?: Date | undefined;
    created_at: Date;
    updated_at: Date;
};
interface StaffType extends MasterStaffType {
    department: StaffDepartmentType;
...
export function AuthProvider({
    ...
    useEffect(() => {
        ...
        if (staffId) {
            ...
            document.cookie = "selected_staff_id=; path=/; max-age=0; samesite=lax";
        }
        ...
        if (staffList.length === 1) {
            ...
            document.cookie = `selected_staff_id=${encodeURIComponent(
                only.id,
            )}; path=/; samesite=lax`;

            const normalized = only.memberships?.[0]?.department
                ? {
                    ...only,
                    department: only.memberships[0].department,
                }
                : only;

            setProfile((prev) => (prev?.id === normalized.id ? prev : normalized));
            return;
        }
